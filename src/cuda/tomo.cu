...
tomo::tomo(size_t ntheta, size_t pnz, size_t n, float center,
...
void tomo::fwd(size_t g_, size_t f_, size_t igpu) {
...
void tomo::adj(size_t f_, size_t g_, size_t igpu, bool filter) {
    cudaSetDevice(igpu);
    float2* g0 = (float2 *)g_;
    for (int i=0;i<ntheta;i++)    
