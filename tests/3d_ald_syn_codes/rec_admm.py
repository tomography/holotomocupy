...
def admm_gpu(pslv, pslv0, tslv, data,data_ref, h, psi, prb,shifts, code, shifts_code, lamd, u, piter, titer, niter,step=1,viz_step=1):
    ...
    for m in range(niter):
        ...
        if m%viz_step==0:
            ...
            dxchange.write_tiff(cp.imag(u[69]).get(), f'/data/vnikitin/modeling/3d_ald_syn_codes{flg}/uim/o{m:05}.tiff',overwrite=True)
            dxchange.write_tiff(cp.real(u).get(),      f'/data/vnikitin/modeling/3d_ald_syn_codes{flg}/ure/r{m:05}.tiff',overwrite=True)
            dxchange.write_tiff(cp.real(u[69]).get(), f'/data/vnikitin/modeling/3d_ald_syn_codes{flg}/ure/o{m:05}.tiff',overwrite=True)
