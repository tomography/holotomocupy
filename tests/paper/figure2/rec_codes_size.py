...
prb00 = tifffile.imread(f'data/prb_abs_{n}.tiff')*np.exp(1j* tifffile.imread(f'data/prb_phase_{n}.tiff'))
prb00[:] = prb[0]

import random
import scipy.ndimage as ndimage

for ill_feature_size in [5e-8,1e-8,1e-5,5e-5]:#1e-6,1e-7, 5e-7,5e-6,2.5e-7, 2.5e-6]:
    ...
    for k in range(2,4):
        ...
        tifffile.imwrite(f'data/rec_amp_{k}codes_{ill_feature_size}.tiff',np.abs(rec))
        tifffile.imwrite(f'data/rec_angle_{k}codes_{ill_feature_size}.tiff',np.angle(rec))
        a = np.angle(rec[0])
        b = np.angle(psi[0])
        a-=np.mean(a)
        b-=np.mean(b)
        ...
        print(f'{ill_feature_size=}, ncodes={k} error={np.linalg.norm(rec-psi)} ssim {score} data/rec_angle_{k}codes_{ill_feature_size}.tiff')
