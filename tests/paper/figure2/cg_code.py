...
def cg_holo(pslv, data, init, prb,  piter, nerr_th):
...
def cg_holo_batch(pslv, data, init, prb, piter, nerr_th):
    """Batch of CG solvers"""
    ...
    for ids in holotomo.utils.chunk(range(pslv.ntheta), pslv.ptheta):
        ...
        # Radon transform
        res_gpu = cg_holo(pslv, data_gpu, init_gpu,prb_gpu, piter, nerr_th)
        # copy result to cpu
        res[ids] = res_gpu.get()
    return res
