...
def cg_holo(pslv, pslv0, data, data_ref, init, init_prb,  piter,shifts, upd_psi=True, upd_prb=False,step=1,vis_step=1):
    ...
    for i in range(piter):
        ...
        if upd_prb:
            ...
            dprb += pslv0.adj_holo_prb(fprb0-cp.sqrt(data_ref)*cp.exp(1j*cp.angle(fprb0)),psi*0+1,shifts*0)
            dprb*=-0.5
            # gradprb0 = gradprb
            # line search
        ...
        if i%vis_step==0:  
            ...
            fig.colorbar(im)
            plt.savefig(f'/data/vnikitin/holo/best/{i:05}.png',dpi=300)
...
rec,rec_prb = cg_holo_batch(pslv, pslv0, data, data_ref, rec, rec_prb, 10000, shifts_rec, True,True,256,256)


# In[ ]:


dxchange.write_tiff(np.abs(rec),f'/data/vnikitin/holo//psirec_chip_best_shift_abs{n}_{ntheta}_{ndist}',overwrite=True)
dxchange.write_tiff(np.angle(rec),f'/data/vnikitin/holo/psirec_chip_best_shift_angle{n}_{ntheta}_{ndist}',overwrite=True)

dxchange.write_tiff(np.abs(rec_prb),f'/data/vnikitin/holo/prbrec_chip_best_shift_abs{n}_{ntheta}_{ndist}',overwrite=True)
dxchange.write_tiff(np.angle(rec_prb),f'/data/vnikitin/holo/prbrec_chip_best_shift_angle{n}_{ntheta}_{ndist}',overwrite=True)
...
dxchange.write_tiff(urec.real,f'/data/vnikitin/holo/urec_chip_best_shift_re{n}_{ntheta}_{ndist}',overwrite=True)
dxchange.write_tiff(urec.imag,f'/data/vnikitin/holo/urec_chip_best_shift_im{n}_{ntheta}_{ndist}',overwrite=True)
