...
def cg_holo(pslv, data, init, init_prb,  piter,shifts, upd_psi=True, upd_prb=False,step=1,vis_step=1):
    ...
    for i in range(piter):
        if upd_psi:
            ...
            d = -pslv.adj_holo(fpsi-cp.sqrt(data)*cp.exp(1j*(cp.angle(fpsi))), prb,shifts)/cp.max(cp.abs(prb))**2#/ndist**2
            # # Dai-Yuan direction
            # if i == 0:
            #     d = -grad
            # else:
        ...
        if upd_prb:
            ...
            dprb = -pslv.adj_holo_prb(fprb-cp.sqrt(data)*cp.exp(1j*cp.angle(fprb)),psi,shifts)/cp.max(cp.abs(psi))**2/pslv.ntheta#/ndist**2
            # gradprb0 = gradprb
            # line search
        ...
        if i%vis_step==0:  
            ...
            fig.colorbar(im)
            plt.savefig(f'/data/vnikitin/holo/twostep/{i:05}.png',dpi=300)
...
_,rec_prb = cg_holo_batch(pslv, data_ref, rec, rec_prb, 32, shifts_ref, False,True,31,10000)
...
# data_ref = ref[:1,np.newaxis]
# shifts_ref = cp.array(shifts/norm_magnifications[:,np.newaxis,np.newaxis])[:1,:1]*0
# _,rec_prb = cg_holo_batch(pslv, data_ref, rec, rec_prb, 32, shifts_ref, False,True,31,31)
...
rec,rec_prb = cg_holo_batch(pslv, data, rec, rec_prb, 10000, shifts_rec, True,True,256,256)


# In[ ]:


dxchange.write_tiff(np.abs(rec),f'/data/vnikitin/holo//psirec_chip_two_shift_abs{n}_{ntheta}_{ndist}',overwrite=True)
dxchange.write_tiff(np.angle(rec),f'/data/vnikitin/holo/psirec_chip_two_shift_angle{n}_{ntheta}_{ndist}',overwrite=True)

dxchange.write_tiff(np.abs(rec_prb),f'/data/vnikitin/holo/prbrec_chip_two_shift_abs{n}_{ntheta}_{ndist}',overwrite=True)
dxchange.write_tiff(np.angle(rec_prb),f'/data/vnikitin/holo/prbrec_chip_two_shift_angle{n}_{ntheta}_{ndist}',overwrite=True)
...
dxchange.write_tiff(urec.real,f'/data/vnikitin/holo/urec_chip_two_shift_re{n}_{ntheta}_{ndist}',overwrite=True)
dxchange.write_tiff(urec.imag,f'/data/vnikitin/holo/urec_chip_two_shift_im{n}_{ntheta}_{ndist}',overwrite=True)
